...
import { loadAudioFile } from "./audio-system";
...
export function loadAssets(_rctx: RenderDeviceWebGL, actx: AudioContext) {
	const assetBaseURL = "./";
	...
	const resources = [
		...
		*/
		...
		loadAudioFile(actx, assetBaseURL + "data/sound/snowloop.mp3").then((buf) => {
			assets.snowLoop = buf;
		}),
		loadAudioFile(actx, assetBaseURL + "data/sound/B-Roll-incompetech.mp3").then((buf) => {
			assets.music = buf;
		}),
		loadAudioFile(actx, assetBaseURL + "data/sound/count1.wav").then((buf) => {
			assets.count1Sample = buf;
		}),
		loadAudioFile(actx, assetBaseURL + "data/sound/count2.wav").then((buf) => {
			assets.count2Sample = buf;
		}),
		loadAudioFile(actx, assetBaseURL + "data/sound/wallhit.wav").then((buf) => {
			assets.wallHitSample = buf;
		}),
		loadAudioFile(actx, assetBaseURL + "data/sound/pickup.wav").then((buf) => {
			assets.pickupSample = buf;
		}),
		loadAudioFile(actx, assetBaseURL + "data/sound/orch.wav").then((buf) => {
			assets.orchHitSample = buf;
		}),
		loadAudioFile(actx, assetBaseURL + "data/sound/fanfare.mp3").then((buf) => {
			assets.fanfareSample = buf;
		}),
		loadAudioFile(actx, assetBaseURL + "data/sound/slam.wav").then((buf) => {
